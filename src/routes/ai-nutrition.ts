...
4. Se o usuário **não tem planos ainda**:
   ...
   - Se sim, colete apenas o que ainda não está no perfil, nessa ordem:
     ...
     2. **Sexo biológico** — necessário para o cálculo de metabolismo. Pergunte com linguagem respeitosa. Se o usuário não quiser informar, explique que o cálculo ficará mais aproximado e prossiga.
     3. **Idade** — somente se não estiver no perfil
     4. **Peso** (kg) — somente se não estiver no perfil
     5. **Altura** (cm) — somente se não estiver no perfil
     6. **Nível de atividade física** — apresente as opções:
     ...
     7. **Dias de treino por semana** e quais dias (para variação treino vs descanso)
     8. **Horários disponíveis para refeições** (ex: acorda às 7h, almoça ao meio-dia, dorme às 23h)
     9. **Quantas refeições consegue fazer por dia** (entre 3 e 6)
     10. **Restrições ou alergias alimentares** (ex: lactose, glúten, frutos do mar)
     11. **Preferências alimentares** — vegetariano, vegano, ou alimentos que não come
     12. **Alimentos que gosta** (para usar no plano sempre que possível)
     13. **Preferência de estilo do plano**:
         - 💰 Econômico — alimentos acessíveis e baratos
         - ⚡ Prático — refeições simples e rápidas de preparar
         - 🎨 Variado — cardápio diversificado com mais opções
...
### Cálculo de macros
- Use **Mifflin-St Jeor** com fator de atividade para calcular o TDEE:
...
### Segurança e limites
- **Nunca gere planos abaixo de 1200 kcal para mulheres ou 1500 kcal para homens**, independente do objetivo.
...
- Use os alimentos que o usuário gosta sempre que possível.
- Evite receitas complexas a menos que o usuário prefira variedade.
      ...
      const result = streamText({
        ...
        tools: {
          ...
          createNutritionPlan: tool({
            description: "Cria um plano nutricional completo com refeições detalhadas.",
            inputSchema: z.object({
              goal: z.enum([
                "cutting",
                "ganho de massa",
              ...
              notes:         z.string().max(300).optional().describe("Observações gerais do plano"),
              ...
              days: z.array(
                z.object({
                  weekDay: z
                    .nativeEnum(WeekDay)
                    .optional()
                    .describe("Omitir se for plano único igual para todos os dias. Nunca repita o mesmo weekDay."),
                  order: z.number().int().nonnegative(),
                  meals: z.array(
                    z.object({
                      name:     z.string().min(1).max(80).describe("Café da manhã, Almoço, Lanche, Jantar, Pré-treino, Pós-treino"),
                      time:     z.string().regex(/^([01]\d|2[0-3]):([0-5]\d)$/).optional().describe("Horário no formato HH:MM ex: 07:30"),
                      calories: z.number().positive(),
                      protein:  z.number().nonnegative(),
                      carbs:    z.number().nonnegative(),
                      fat:      z.number().nonnegative(),
                      order:    z.number().int().nonnegative(),
                      notes:    z.string().max(300).optional().describe("Substituições sugeridas"),
                      foods: z.array(
                        z.object({
                          name:     z.string().min(1).max(80),
                          quantity: z.number().positive(),
                          unit:     z.string().min(1).max(20).describe("g, ml, unidade, colher, xícara"),
